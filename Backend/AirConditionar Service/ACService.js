...
function getAirConditionarById(id, res){
	...
	con.query(productQuery, (err, resp) => {
		...
		con.query(imageQuery, (_, imageResp)=>{
			var prod = resp[0]
			const product = new airConditionar(prod.id, prod.name, prod.price, prod.discount, imageResp, prod.features, prod.about)
...
module.exports = {getAirConditionarById, getACProductsByPage, addProduct}