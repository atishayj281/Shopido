...
con.connect(function (err) {
	if (err) throw err;
	console.log("cloths Service Connected");
});

function getClothById(id, res){
	...
	con.query(productQuery, (err, resp) => {
		...
		con.query(imageQuery, (_, imageResp)=>{
			var prod = resp[0]
			const product = new cloths(prod.id, prod.name, prod.price, prod.discount, imageResp, prod.features, prod.about)
			res.status(200).json({ data: product });
		})
	});
}

function getClothsByPage(page, res) {
	var pageSize = 10;
...
function addProduct(id, name, description, price, discount, rating, highlights, images, features, req, res) {
	...
	con.query(query1, (err, resp) => {
		...
		con.query(query2, (error, response) => {
			...
			console.log(`Product ${id} of Cloths added Successfully`);
			//res.status(response.code).json({message: `Product ${id} of Cloths added Successfully`})
		})
	})
}

module.exports = {getClothById, getClothsByPage, addProduct}