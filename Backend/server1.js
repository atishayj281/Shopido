...
app.post('/updateProfile', (req, res) => {
	console.log("ok")
	var body = req.body
	if(body == undefined) res.status(400).json({message: "body can not be empty"});
	else {
		profileServices.updateCustomer(body, req, res);
	}
})
...
app.post("/getOrderedProductsByOrderId", (req, res) => {
	...
	orderService.getOrderDetailsByOrderId(cust_id, orderId, req, res)
})

app.get('/getProductById', (req, res)=>{
	ACService.updateStock(1);
	console.log(req.url)
...
app.post("/createOrder", (req, res) => {
	...
	if(!body || body == undefined) {
		...
		paymentService.createOrder(body, res);
	}

})

app.post("/getKEY", (req, res) => {
	paymentService.getAPI_KEY(res);
})

app.post("/getUserId", (req, res) => {
	profileServices.getUserId(req.body.email, req, res);
})
