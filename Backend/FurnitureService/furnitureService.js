...
con.connect(function (err) {
	if (err) throw err;
	console.log("furniture Service Connected");
});
...
function getFurnitureById(id, res){
	...
	con.query(productQuery, (err, resp) => {
		...
		con.query(imageQuery, (_, imageResp)=>{
			var prod = resp[0]
			const product = new furniture(prod.id, prod.name, prod.price, prod.discount, imageResp, prod.features, prod.about)
...
module.exports = {getFurnitureById, getPageProducts, addProduct, updateStock}